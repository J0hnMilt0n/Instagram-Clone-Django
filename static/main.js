    ...
    likeUnlikeForms.forEach(form => form.addEventListener('click', e => {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                document.getElementById(`likes-${clickId}`).textContent = response.count;
                if(response.liked) {
                    clickedBtn.innerHTML = '<i class="bi bi-heart" id="like"></i> like';
                }else {
                    clickedBtn.innerHTML = '<i class="bi bi-heart-fill" id="unlike"></i> unlike';
                }
            },
            ...
            <div class="card-body">
                <div className="btns">
                    <form class="d-inline like-unlike-form" form-like-id=${id}>
                         <button class="border-0 pl-2 pr-2 btn " id="like-unlike-${id}" >${liked ? "<i class='bi bi-heart-fill' id='unlike'></i> unlike":"<i class='bi bi-heart' id='like'></i> like"}</button>
                    </form>
                </div>
